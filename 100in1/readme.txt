100in1ロムのファームウェアです。
最大100のROMファイルをMSXπのフラッシュメモリに格納する事が出来ます。
ROM格納用のフラッシュメモリ領域は16Mバイト弱です。
100in1ロムファイルの作成についてはtool以下のreadme.txtを読んでください。
...
1. 100in1.binファイルの後ろに使用する100in1ロムファイルを連結します。

unixのcatコマンドを使用する例
$ cat 100in1.bin roms.bin > firmware.bin

windowsのcopyコマンドを使用する例
>copy /b 100in1.bin + roms.bin firmware.bin
...
MSX起動時にInput ROM#:と表示されたらROM番号(00～99)を入力しリターンキーを押してください。
番号に対応したゲームが起動します。
